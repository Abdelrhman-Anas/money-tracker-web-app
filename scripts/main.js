...
// the html will be generated by JS according to what page the user need to use in these elements 
...
function thePage() {
  if (page === 0) {
    dash(language, totalMoney, currency, bodyElement, indeElement, trackerIn, trackerOut);
  } else if (page === 1) {
    trans(bodyElement, indeElement, trackerIn, trackerOut, language, currency);
  } else if (page === 2) {
    repo(bodyElement, indeElement, Chart, language, trackerIn, trackerOut);
  } else if (page === 3) {
    sitt();
  }
};
